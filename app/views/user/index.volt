...
<div class="panel panel-default">
	...
	<div id="collapseThree" class="panel-collapse collapse" style="height: auto;">
		<div class="panel-body">
			{{ form('/backend_app/user/', 'method': 'get', 'enctype': 'multipart/form-data') }}
...
<span class="pull-right" style="margin-right:14px;margin-bottom:14px;">
	<a href="/backend_app/user/new/"><i class="fa fa-link" aria-hidden="true"></i>&nbsp;新規登録はこちら</a>
</span>
		...
		{% if user is not empty %}
			...
			{% endif %}
					<tr>
						<td>{{ user.id }}</td>
						<td>{{ user.name }}</td>
						<td style='word-break: break-all;'>{{ user.mail }}</td>

						<td>
							{{ form('backend_app/user/edit?id=' ~ user.id ~ '/', 'method': 'post') }}
						...
						<td>
							{{ form('backend_app/user/delete?id=' ~ user.id ~ '/', 'method': 'post', 'onSubmit': 'return delete_check()') }}
